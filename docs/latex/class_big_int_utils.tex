...
\begin{DoxyCompactList}\small\item\em This method is used to subtract one \doxyref{Big\+Integer}{p.}{class_big_integer} variable from another. \end{DoxyCompactList}\end{DoxyCompactItemize}
...
\end{DoxySeeAlso}
This method is used to iterate through all of the values in both arrays and add them. It starts from the end of both arrays and adds them a digit at a time. It then carries over any additional value and adds that to the next addition. It does this until there are no more numbers to add and returns this value. This method is used by the Program and isn\textquotesingle{}t used by the user. \mbox{\label{class_big_int_utils_a6f355ec25a51cf1f480966d6d0780ea4}} 
...
This method is used to subtract one \doxyref{Big\+Integer}{p.}{class_big_integer} variable from another. 


\begin{DoxyParams}{Parameters}
{\em first\+Num} & is the host \doxyref{Big\+Integer}{p.}{class_big_integer} variable to have another value subtracted from it. \\
\hline
{\em second\+Num} & is the \doxyref{Big\+Integer}{p.}{class_big_integer} variable that is subtracted from the host variable. \\
\hline
{\em result} & is the result of the longest array as well as the shortest, the difference between them, and which of the two is longest. \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}

\end{DoxyReturn}
This method is used to correctly subtract the second\+Num from the first\+Num. It iterates through both arrays subtracting the individual digits one at a time until they have reached the end of both arrays. It then returns the value to a \doxyref{Big\+Integer}{p.}{class_big_integer} result. This program is used by the program and not by the user. 
